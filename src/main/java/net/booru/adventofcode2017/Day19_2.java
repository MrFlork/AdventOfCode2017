...
class Day19_2
{
    static void run() throws IOException
    {
        ...
        final int value = part2(parseMap(Files.readAllLines(Paths.get("day19.in"))));

        System.out.println("part2: " + value);
    }

    static int part2(final Map map)
    {
        final int startX = getStartX(map);
        final int count = follow(new State(new Position(startX, 0), Direction.Down), map);
        return count;
    }
    ...
    private static int follow(final State state, final Map map)
    {
        final State nextState = state.follow(map);

        if (!nextState.isEndState())
        {
            return 1 + follow(nextState, map);
        }
